 ...
 * @package Newstalk
 */
if ( ! function_exists( 'newstalk_enqueue_styles' ) ) :
	/**
	 * @since 0.1
	 */
	function newstalk_enqueue_styles() {
		wp_enqueue_style( 'newsup-style-parent', get_template_directory_uri() . '/style.css' );
		wp_enqueue_style( 'newstalk-style', get_stylesheet_directory_uri() . '/style.css', array( 'newsup-style-parent' ), '1.0' );
		wp_enqueue_style('bootstrap', get_template_directory_uri() . '/css/bootstrap.css');
		wp_dequeue_style( 'newsup-default',get_template_directory_uri() .'/css/colors/default.css');
		wp_enqueue_style( 'newstalk-default-css', get_stylesheet_directory_uri()."/css/colors/default.css" );
		if(is_rtl()){
...
add_action( 'wp_enqueue_scripts', 'newstalk_enqueue_styles', 9999 );

function newstalk_theme_setup() {

//Load text domain for translation-ready
load_theme_textdomain('news-talk', get_stylesheet_directory() . '/languages');
...
// custom header Support
		...
		'top_right' => __( 'Top Header menu', 'news-talk' ),
		) );
} 
add_action( 'after_setup_theme', 'newstalk_theme_setup' );


add_action( 'customize_register', 'newstalk_customizer_rid_values', 1000 );
function newstalk_customizer_rid_values($wp_customize) {
 ...
 */
function newstalk_widgets_init() {
	

	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Left Sidebar Section', 'news-talk'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Right Sidebar Section', 'news-talk'),
...
add_action( 'widgets_init', 'newstalk_widgets_init' );


function newstalk_remove_some_widgets(){
// Unregister Frontpage sidebar
unregister_sidebar( 'front-page-sidebar' );
}
add_action( 'widgets_init', 'newstalk_remove_some_widgets', 11 );

function newstalk_menu(){ ?>
<script>
...
<?php }
add_action( 'wp_footer', 'newstalk_menu' );